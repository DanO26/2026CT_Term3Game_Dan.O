...
[MY GAME VIDEO](./Video%20Walkthrough%20My%20Game.mp4)
<!--
...
|---|---|---|
|[Camera Snapping System](./Showing%20camera%20snap.mp4) |This clip shows my camera snapping system |C:\Users\dan.ozeri\OneDrive - NSW Department of Education\Documents\GitHub\2026CT_MyGame_Dan.O\Showing camera snap.mp4 |
|[BounceBack System](./Showing%20BounceBack.mp4) |This clip shows my bounceback syste, This system applies when the player hits a wall with a certain velocity, when the player hits the wall they are pushed back with around half the velocity that the pushed the wall with.  |C:\Users\dan.ozeri\OneDrive - NSW Department of Education\Documents\GitHub\2026CT_MyGame_Dan.O\Showing BounceBack.mp4 |
| | | |
...
|---|---|---|
|Retro synth - 80/90's/ Starting Game |Fan Zoo/ Unity assets | |
| | | |
...
| Feature | Description |
|---|---|
| Scene Loading Method | On activation when "play" button is pressed, the scene switches instantly to Castle Jumpers. |
| Persistent Data Between Scenes | There is nothing for the player input that would carry over so it is either lost or none|
...
|---|---|---|
|PlayerMovement.cs | Player | my player movement script is the responsible for many things such as the movement of the player left and right, Bounceback when hitting walls, Ice level physics, charging and releasing jumps and Ground check|
|CameraFollow.cs| Camera | My camera scipt is responsible for keeping the camera locked onto the player, Updates the camera position based on the players movement, Vertical snapping, screenHeight, snapBuffer, and snapSpeed |
|GameController.cs|Spikes | This script is the main funtion for death when hitting spikes and responsible for the repawn function when the player dies. |
|Menu| Menu | Introducing the player to the game and the first screen the player see's, responsible for starting and quiting the game.|
| | | |

### 8.2 Key Algorithms / Logic
| Feature | Script | Description |
|---|---|---|
|Ground check |PlayerMovement.cs | Uses an OverlapCircle under the player to determine grounded state, enabling jumps, charging, and grounded movement.|
| Charge Jump|PlayerMovement.cs |Increases jump power over time while holding space, then releases a charged jump based on stored force, This algorithm controls three phases which are starting jump, Building charge and Releasing charge. Each phase is triggered by different input states KeyDown, Key, KeyUp and uses rigidbody2D to apply the final jump force. |
|Bounce Back |PlayerMovement.cs | BounceBack makes the player rebound when hitting walls, usally when failing a jump. Bounceback activates when the player is grounded, the player is moving horizontally into a wall and a wall detection ray hits a collider on the ground layer. When these conditions apply the player is pushed left when hitting a wall from the right and pushed right when hitting a wall left. |
| Ice Platform| Player.cs | the ice physiscs on the platforms reduce friction and preserve sideways momentum this causes the player to slide with higher linear velocity.x than normal. When this increased momentum carries the player into a wall, the wall detection raycasts register the collision and the bounce back algorithm reverses the horizontal velocity multiplying it to create a sharp rebound. Because the ice continues to preserve momentum after the reversal the bounce lasts longer and travels farther making wall collisions on ice significantly more punishing.|
| Screen snap| CameraFollow.cs | The snap threshold logic calculates a vertical cutoff point that determines when the camera should move to the next screen. The threshold is computed as screenHeight amd snapBuffer where screenHeight matches the cameras visible height (13 units in your game) and snapBuffer adds extra space above the top of the screen. Each frame, the camera checks whether the players Y position has risen above this threshold relative to the current camera anchor. If the player stays below it the camera remains locked creating stable one screen rooms. Once the player crosses the threshold the camera begins a controlled snap upward using your snapSpeed shifting the entire view to the next vertical section.|

### 8.3 Design Patterns Used
| Pattern | Where Applied | Justification |
|---|---|---|
| Jump State| Player| My movement script uses the a state pattern by controlling the player through a set of mutually exclusive states for example when isCharging becomes true the player enters the charging state, which freezes horizontal movement and begins building jump power. When the jump is released, the player transitions into the jumping state, where vertical velocity is applied and charging logic is disabled. If the player hits a wall the script switches into the bounce back state this reverses the horizontal velocity and temporarily disabling input. when the player is touches ice activates the ice state modifying friction and momentum, the spike collision triggers the respawn state disabling physics this teleports the player into the default state|
|Camera |Camera | The patterns used in my game is a threshhold that decided when the camera will swith screens, when the player hits and goes over a certain threshold which is 13 units plus a small buffer. Each frame my camera checks whether the players y-position has risen above this threshold, if not then the camera does not move and if the player has not gone above this threshohold, the camera will stay locked onto the player.|

| Collision State| Player|When the ground check circle detects a platform, the player enters the grounded state, enabling charging and horizontal movement. When the wall raycasts hit a left or right wall, the player enters the bounce back state this reverses the horizontal velocity and disables input. When the player touches an object with the ice tag the collider swithes the player into an ice state, this state reduces frction and preserves momentum.|
...
|---|---|---|---|---|---|
| 1 |2D Player Movement In Unity | bendux |https://www.youtube.com/watch?v=K1xZ-rycYY8 |This toturail was used for the bases of movement in my game such as left right and jumping movement states. |In my game the player moves faster than the player in the toturial, I also added the charge jump mechanic and when this mechainc activates the player cannont move horizontilty|
| 2 |START MENU in Unity |Brackeys |https://www.youtube.com/watch?v=zc8ac_qUXQY | A menu at the start of my game | I removed the options button becuase i did not need it and it was not vital in creating a game of this size|
| 3 |How to add MUSIC in Unity in 30 seconds (NO CODE) | GDT Solutions |https://www.youtube.com/watch?v=hiA_qRiNgfg |Music while the player plays the game. |In my game i used different music it fit the theme of my game |
| 4 |2D Animation in Unity (Tutorial) |Brackeys |https://www.youtube.com/watch?v=hkaysu1Z-N8 | Player sprite animation |I used a different sprite in my game than in the toturoial. |
| 5 |Unity 2D Player RESPAWN Tutorial | Unity 2D Platformer Tutorial #8 |
Rehope Games |https://www.youtube.com/watch?v=odStG_LfPMQ |To apply a respawn after my player runs into a spike |Changed repawn time and threshold |
| 6 |Long press for charged Jump |NolanOc| https://discussions.unity.com/t/long-press-for-charged-jump/202543| The basis for my charged jump |Added a limit of not moving horizontaly when charging jump/ holding space. | 
| 7 | How to add Collisions in Unity| |https://www.youtube.com/watch?v=yL-wT6kOK84 | Collions for wall, so boucne back could function an the player had groudn to stand on | Changed wall size, colliosion threshold|
| 8 |Import a sprite or spritesheet texture |Unity |https://docs.unity3d.com/6000.6/Documentation/Manual/sprite/import-images-sprites/import-images-sprites-landing.html|Importing backgrounds and background sprites/assests |Used different sprites, scaling spites up and down in size |
...
|---|---|---|---|---|---|
|Free 2D Mega Pack |Sprite |Brackeys- Unity assests |Standard Unity Asset Store EULA |https://assetstore.unity.com/account/assets |Background and Backgroud sprites |
|Slime Enemy - Pixel Art |Player sprite |War- Unity assests  |Standard Unity Asset Store EULA |https://assetstore.unity.com/account/assets |Player sprite |
| | | | | | |

### 10.2 Audio Assets
| Asset Name | Type | Creator / Source | Licence | URL | Used For |
|---|---|---|---|---|---|
|Retro synth - 80/90's |Audio/Music|Fan Zoo - Unity assests |Standard Unity Asset Store EULA |https://assetstore.unity.com/packages/audio/ambient/sci-fi/retro-synth-80-90-s-191422 |Menu music |
| | | | | | |
| | | | | | |
...
|---|---|---|---|---|---|
|Player.cs  |bendux | |https://gist.github.com/bendux/5fab0c176855d4e37bf6a38bb071b4a4 |Non-Exclusive worlwide license (Github Unlicensed) |This code was used for the bases of player movement eg. moveing left, right and up |Added a charge jump, changed speed, when charging jumpps the player cannot move
|Menu.cs |Brackeys |General Non Exclusive (Free) |https://www.youtube.com/watch?v=zc8ac_qUXQY |Start menu |Changed font size, buttons and colour/theme |
|Variable values|Outrage||Creative Commons|https://www.youtube.com/watch?v=o4Z99Xi5xXc|The values for movemtn and jumping, did not keep the rest the code|


### 10.4 Unity Packages & Plugins
| Package Name | Version | Source | Licence | URL | Purpose |
|---|---|---|---|---|---|
|2D |3.0.1 |Unity | Unity Companion License |https://docs.unity3d.com/6000.1/Documentation/Manual/2DFeature.html |2D development |
|TextMeshPro |3.20 |Unity |https://docs.unity3d.com/Packages/com.unity.ugui@2.0/manual/TextMeshPro/index.html |Fonts/ was used for Start menu | |
| | | | | | |

### 10.5 Fonts
| Font Name | Creator / Source | Licence | URL |
|---|---|---|---|
|TextMeshPro |Unity |Standard Unity Asset Store EULA  |https://docs.unity3d.com/Packages/com.unity.ugui@2.0/manual/TextMeshPro/index.html |
| | | | |
...
|---|---|---|
| 1 |The scaling of map size relative to the player and camera snapping |I scaled everything down all objects and player and expiremented with camera snapping so that it would feel right for my game |
| 2 |Player move really slowly and could only slide left or right rely slow, would shake |Remade the whole script through the tutorial|
| 3 |Bounceback did not work and would not bounce off on walls sideways  |Changed how ground check checks y-level, changed types of platforms used |
| 4 |Ice Platforms would not keep momentum when the player would jump onto them, when they jumped onto them the platfrom would just stop them and then apply the accelerated momentum |Changed the way the iceplatfroms input momentum and added extra acceleration onto the platfroms|
| 5 |Player could still move when charging jump, this is not wanted|Disabled x-level movement when "spacebar" is held, and added momentum in the air so the player could aim where they wanted to land |
